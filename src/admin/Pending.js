...
function Pending() {
  ...
  const fetchComplaints = () => {
    ...
    axios.get('http://localhost:3000/complaints/all', {headers}).then((response) => {
      let pendingComplaints = response.data[0];
      setComplaints([...pendingComplaints.Vip, ...pendingComplaints.nonVip])
    });
  }
  ...
  const pending = complaints === 'there are no complaints!' ? [] : complaints
  .filter((complaint) => {
