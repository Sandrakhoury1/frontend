...
function Resolved() {
  ...
  const resolved = complaints === 'there are no complaints!' ? [] : complaints
  .filter((complaint) => {
  ...
  const fetchComplaints = () => {
    ...
    axios.get('http://localhost:3000/complaints/all', {headers}).then((response) => {
      let resolvedComplaints = response.data[0];
      setComplaints([...resolvedComplaints.Vip, ...resolvedComplaints.nonVip])
    });
  }
